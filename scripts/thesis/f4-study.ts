...
async function main() {
  ...
  try {
    ...
    if (everyConfigBelowBaselineRel) {
      ...
      rows.push(`**Attribution caveat (important):** the headline −relevance% conflates TWO effects. The baseline is 4-source RRF fusion (retrieval+npmi+popular+exploration), but the scorer's relevance FEATURE is a single signal (cosine to the user's modes ≈ retrieval source only). So the gap from the relevance-only config (rel=1, all else 0) down to the baseline is largely a single-signal-vs-fusion gap — NOT a trade-off cost — while the gap from the relevance-only config down to the revenue corner is the TRUE revenue trade-off cost. A fairer future baseline would score the relevance-only multi-objective config against itself, or add the npmi/popularity signals as relevance features.`, "");
    } else {
