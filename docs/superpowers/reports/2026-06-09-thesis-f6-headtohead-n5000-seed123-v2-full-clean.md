...
In the **full** frame, the relevance-optimal pipeline config (**popular-cohort-real**) scores nDCG@10 **0.016** vs popular-cohort's **0.262** — a -93.8% DEFICIT (and revenue@10 +592.6% at the same time).
...
**Verdict (full, n=5000, seed=123): even the relevance-optimal pipeline config (popular-cohort-real) does NOT beat popular-cohort on nDCG@10 (-93.8%).** Reported as-is per F6's honesty mandate. The cohort=subcategory rival is strong on a synthetic catalog of n=5000; W2 (scale) tests whether the cohort dilutes as the catalog grows.
